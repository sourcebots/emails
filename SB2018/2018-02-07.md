...
The `polar` coordinate system has been replaced with [`spherical`](https://docs.sourcebots.co.uk/api/vision/coordinates/#spherical-coordinates). The new system is more accurate, and better documented than polar, and we recommend updating to it as soon as possible! The available attributes are identical in `polar`, and so will likely be a simple drop-in replacement. Be sure to consult the documentation on exactly now to use the new system. Attempting to use `polar` will still work as it does now, however displays a warning message.
